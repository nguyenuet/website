...
Khi bạn triển khai Kubernetes, bạn có một cluster.
{{< glossary_definition term_id="cluster" length="all" prepend="A Kubernetes cluster consists of">}}

This document outlines the various components you need to have
a complete and working Kubernetes cluster.
Tài liệu này miêu tả nhiều thành phần bạn cần để có một Kubernetes cluster hoàn chỉnh.

Here's the diagram of a Kubernetes cluster with all the components tied together.
Đây là hình ảnh mô tả một Kubernetes cluster với tất cả thành phần liên kết cùng nhau.
...
Các thành phần ở tầng điểu khiển (Control Plane) thực hiện các quyết định chung (ví dụ, lập lịch), cũng như phát hiện và đáp ứng các sự kiện trong cluster (ví dụ, khởi động một {{< glossary_tooltip text="pod" term_id="pod">}} mới khi trường `replicas` của deployment đang không thỏa mãn)

Control Plane components can be run on any machine in the cluster. However,
for simplicity, set up scripts typically start all Control Plane components on
the same machine, and do not run user containers on this machine. See
[Building High-Availability Clusters](/docs/admin/high-availability/) for an example multi-master-VM setup.
Những thành phần của Control Plane có thể chạy ở bất kì machine nào trong cluster. Tuy nhiên, để đơn giản, những kịch bản cài đặt thường bắt đầu với tất cả các thành phần của Control Plane trên cùng một machine, và không chạy các container của người dùng trên machine này. Tham khảo một ví dụ về cài đặt multi-master-VM [Building High-Availability Clusters](/docs/admin/high-availability/) 
...
[cloud-controller-manager](/docs/tasks/administer-cluster/running-cloud-controller/) chạy các controller cái tương tác với các nhà cung cấp cloud bên dưới. cloud-controller-manager binary là một tính năm alpha đã được giới thiệu tại Kubernetes release 1.6.

cloud-controller-manager runs cloud-provider-specific controller loops only. You must disable these controller loops in the kube-controller-manager. You can disable the controller loops by setting the `--cloud-provider` flag to `external` when starting the kube-controller-manager.
cloud-controller-manager chạy cloud-provider-specific
